...
# (風速,大氣壓力,溫度,濕度,光照度)
# (發電量)
Regression_X_train = SourceData[['Pressure(hpa)', 'Temperature(°C)', 'Humidity(%)', 'Sunlight(Lux)']].values
Regression_y_train = SourceData[['Power(mW)']].values

# LSTM 選擇要留下來的資料欄位
# (風速,大氣壓力,溫度,濕度,光照度)
AllOutPut = SourceData[['Pressure(hpa)', 'Temperature(°C)', 'Humidity(%)', 'Sunlight(Lux)']].values
...
# CNN 部分
regressor.add(Conv1D(filters=64, kernel_size=3, activation='relu', input_shape=(X_train.shape[1], 4)))
regressor.add(MaxPooling1D(pool_size=2))
regressor.add(Dropout(0.2))

# BI-LSTM 部分
regressor.add(Bidirectional(LSTM(units=128, return_sequences=True)))
...
regressor.save('WeatherCNN_BILSTM_' + NowDateTime + '.h5')
print('Model Saved')
...
#============================預測數據============================
# 載入模型
regressor = load_model('WeatherCNN_BILSTM_2024-11-27T19_13_34Z.h5')
Regression = joblib.load('WheatherRegression_2024-11-27T21_27_15Z')
...
while count < len(EXquestion):
    ...
    ReferData = SourceData[[ 'Pressure(hpa)', 'Temperature(°C)', 'Humidity(%)', 'Sunlight(Lux)']].values
